                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-2 mt-2">
                                        <input type="text" class="form-control @error('test_loinc_scale')
                                            ...
                                            id="test_loinc_scale" placeholder="Scale" readonly>
    ...
    $(document).ready(function() {
        $('#test_resulttype').change(function() {
            ...
            if (scale == "Qn") {
                ...
                document.getElementById('Other').style.display = 'none';
                //Get Option id="test_normal_general_ord" from table loinc_answers where loinc_code=test_loinc_code value
                var test_loinc_code = document.getElementById('test_loinc_code').value;
                $.ajax({
                    url: "/data-loinc/loincanswers/" + test_loinc_code,
                    type: "GET",
                    dataType: "json",
                    success: function(data) {
                        console.log(data);
                        const dropdowns = [
                            '#test_normal_general_ord',
                            '#test_normal_male_ord',
                            '#test_normal_female_ord',
                            '#test_normal_child_ord',
                            '#test_normal_baby_ord'
                        ];
                        dropdowns.forEach(selector => $(selector).empty());
                        $('#test_normal_general_ord').append(
                            '<option value="">- Select -</option>');
                        $('#test_normal_male_ord').append(
                            '<option value="">- Select -</option>');
                        $('#test_normal_female_ord').append(
                            '<option value="">- Select -</option>');
                        $('#test_normal_child_ord').append(
                            '<option value="">- Select -</option>');
                        $('#test_normal_baby_ord').append(
                            '<option value="">- Select -</option>');
                        $.each(data, function(key, value) {
                            $('#test_normal_general_ord').append(
                                '<option value="' + value.answer_display_text +
                                '">' +
                                value.answer_display_text +
                                '</option>');
                            $('#test_normal_male_ord').append(
                                '<option value="' + value.answer_display_text +
                                '">' +
