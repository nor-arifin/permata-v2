                            ...
                            <tr class="text-center">
                                ...
                                @else
                                    <td class="text-center">
                                        ...
                                        <form action="" method="POST" class="ml-2" id="sendRequest">
                                            <input type="hidden" name="_method" id="_method" value="PUT" />
                                            <input type="hidden" name="_token" id="_token" value="{{ csrf_token() }}" />
                                            <input type="hidden" name="id" id="id_data" value="{{$sample->id}}" />
                                            <input type="hidden" name="visit_time_sampling" id="visit_time_sampling" value="{{ date('Y-m-d\TH:i:sP') }}" />
                                            <input type="hidden" name="visit_sampling_by" id="visit_sampling_by" value="{{ Auth::user()->name }}" />
                                            <input type="checkbox" class="form-control mt-3" name="service_request" value="requested" id="serviceRequest">
                ...
                check.on('change', function() {
                    ...
                    if (check) {
                        $.ajax({
                            ...
                            // url: 'http://localhost:8000/lab/makeservicerequest/'+id,
                            // type: 'POST',
                            // data: $('form').serialize(), // Remember that you need to have your csrf token included
                            // dataType: 'json',

                            url: '{{ route('lab.sendrequest') }}',
                            type: "POST",
                            data: {
                                'id': id,
                                '_method': $('#method').val(),
