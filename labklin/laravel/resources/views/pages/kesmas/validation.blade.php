...
@section('title', 'Validation Kesmas Result ')
...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>Validation Kesmas - {{ $order->order_type }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">Dashboard</a></div>
                <div class="breadcrumb-item"><a href="#">Labkesmas</a></div>
                <div class="breadcrumb-item">Validate Result {{ $order->order_type }}</div>
            </div>
        </div>
        <div class="section-body">
            ...
            <div class="card">
                <div class="card-header">
                    <h4>Validate Kesmas Result - {{ $order->order_type }}</h4>
                </div>
                ...
                <div class="card-body">
                    {{-- BUTTON SAVE RESULT --}}
                    @if ($order->order_validate_user == null)
                    <div class="row">
                        <div class="col-12 text-right">
                            <input type="textarea" name="note_verify" id="note_verify" value="{{ $order->order_note }}"
                                class="form-control" placeholder="Note" style="width: 100%;" readonly>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-12 text-left">
                            <span class="text-right">This result verifeid at {{ $order->order_verify }} by
                                {{ $verificator->name }}</span>
                        </div>
                    </div>
                    <div class="row mt-3">
                        <div class="col-12 text-right">
                            <button class="btn btn-warning mt-2 mb-2" id="Draft{{ $order->idcode }}">Draft LHU
                            </button>
                            <button class="btn btn-success mt-2 mb-2" id="saveModal{{ $order->idcode }}">Validate Result
                            </button>
                        </div>
                    </div>
                    @else
                    <div class="row mt-3">
                        <div class="col-12 text-left">
                            <span class="badge badge-info">This FPPS Result is released.</span>
                        </div>
                    </div>
                    @endif
                ...
                <div ss="card-body">
                    <div class="row m-3">
                        <div class="col-12 text-center">
                            <button class="btn btn-danger" id="cancelModal"><i class="fa fa-trash"></i> Cancel
                                Result</button>
                        </div>
                    </div>
                </div>
...
document.getElementById('saveModal{{ $order->idcode }}').addEventListener('click', function() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            var url = '/labkesmas/updatevalidate/:idcode';
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    setTimeout(function() {
                        window.location.href = '/labkesmas/validity';
                    }, 1000);
                },
...
<!-- Script for load draft -->
<script>
document.getElementById('Draft{{ $order->idcode }}').addEventListener('click', function() {
    Swal.fire({
        title: 'Are you sure?',
        text: "You are about to view draft this result.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3800b1',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, view draft!'
    }).then((result) => {
        if (result.isConfirmed) {
            //Get idcode from #saveModal $order->idcode
            var idcode = '{{ $order->idcode }}';
            var url = '/labkesmas/draft/:idcode';
            url = url.replace(':idcode', idcode);
            $.ajax({
                //New Tab to open url
                url: url,
                ...
                success: function(response) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Success',
                        text: 'Draft LHU is opened in new tab.',
                    });
                    setTimeout(function() {
                        window.open('/labkesmas/draft/' + idcode, '_blank');
                    }, 1000);
                },
                error: function(response) {
                    Swal.fire({
...
<!-- Script for cancel result with form reason cancelation -->
<script>
document.getElementById('cancelModal').addEventListener('click', function() {
    Swal.fire({
        title: 'Are you sure?',
        text: "You are about to cancel this result.",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3800b1',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            Swal.fire({
                ...
                input: 'textarea',
                inputLabel: 'Please input the reason for cancelation',
                ...
                if (result.isConfirmed) {
                    var idcode = '{{ $order->idcode }}';
                    var url = '/labkesmas/cancel/:idcode';
                    url = url.replace(':idcode', idcode);
                    $.ajax({
                        url: url,
                        type: 'PUT',
