...
class AnamnesesController extends Controller
{
    ...
    public function store(Request $request){
        ...
        // dd($bodyTemperature);
        //POST Agnostic
        [$statusCodeObs, $responseObs] = $client->ss_post($ResourceObservation, $bodyHeartRate);
        [$statusCodeObs, $responseObs] = $client->ss_post($ResourceObservation, $bodyRespiratoryRate);
        [$statusCodeObs, $responseObs] = $client->ss_post($ResourceObservation, $bodySystolic);
        [$statusCodeObs, $responseObs] = $client->ss_post($ResourceObservation, $bodyDiastolic);
        [$statusCodeObs, $responseObs] = $client->ss_post($ResourceObservation, $bodyTemperature);
        // dd($statusCodeObs, $responseObs);
        if($statusCodeObs == 201){

            //GET ID OBSERVATION AS UUID
            $observation_id = $responseObs->id;
            //Update observation visit
            $visit = Visit::findOrFail($idvisit);
            $visit->visit_observation_id = $observation_id;
            $visit->save();
